...
class AddRecipe extends Component {
  ...
  doSearch(term) {
    var link = 'https://api.nal.usda.gov/ndb/search/?format=json&q=' + term + '&sort=n&max=25&offset=0&api_key=INSERT USDA API KEY HERE';
  ...
  addItemToSpecificIngredientsList() {
    ...
    if (this.state.selectedItem == null) {
      ...
      var link='https://api.nal.usda.gov/ndb/nutrients/?format=json&api_key=INSER USDA API KEY HERE&ndbno=' + this.state.selectedItem.ndbno + '&nutrients=203&nutrients=204&nutrients=205&nutrients=208&nutrients=307&nutrients=601&nutrients=605';
      fetch(link).then(function(response) {
        return response.json();
      }).then(function(jsonData) {
        console.log(jsonData);
